...
export const GET: APIRoute = async () => {
  const mods = import.meta.glob<{ frontmatter: any; rawContent: () => string; file: string }>(
  ...
  const items = Object.values(mods)
    ...
    .map(p => {
      ...
      const s     = allSeries[fm.series as keyof typeof allSeries];
      const raw   = typeof p.rawContent === 'function' ? p.rawContent() : '';
      const words = raw.trim().split(/\s+/).length;
      return {
        title:       fm.title ?? '',
        description: fm.description ?? '',
        series:      fm.series ?? '',
        seriesLabel: s?.label ?? fm.series,
        url:         `/${fm.series}/${slug}/`,
        body:        raw
          .replace(/#{1,6}\s/g, ' ')
          .replace(/\*\*/g, '')
